        ...
        for port in ports:
            if port.vid in self.allowed_vids and port.pid in self.allowed_pids:
                if port.device not in self.active_scanners:
                    node = ScannerNode(port.device, port.hwid, self.message_queue, self.storage, manager=self)
                    
                    # FIRST SCANNER GETS THE AD USER
                    if len(self.active_scanners) == 0 and self.ad_fallback_name:
                        node.user = self.ad_fallback_name
                        node.ad_fallback_name = self.ad_fallback_name
                        
                    self.active_scanners[port.device] = node
