...
import csv
import os
import json # <-- Dodano do bazy pracowników
...
class CsvStorage:
    def __init__(self, base_path="inventory.csv"):
        ...
        self.employees_file = "employees.json" # <-- Plik z listą pracowników
        
        self.headers = ["date_or_day", "time", "location_id", "sample_id", "sample_name", "desc/notes", "user"]
        self._ensure_employees_file()

    def _ensure_employees_file(self):
        """Tworzy czysty plik pracowników, jeśli jeszcze nie istnieje"""
        if not os.path.exists(self.employees_file):
            try:
                with open(self.employees_file, "w", encoding='utf-8') as f:
                    # Przykładowy pracownik na start
                    json.dump(["ADMIN"], f) 
            except Exception:
                pass

    def get_employees(self):
        """Pobiera listę pracowników z pliku"""
        try:
    ...
    def add_employee(self, name):
        """Dodaje nowego pracownika do bazy i zapobiega duplikatom"""
        emps = self.get_employees()
        if name not in emps:
            emps.append(name)
            # Sortujemy alfabetycznie dla wygody
            emps.sort()
            try:
                with open(self.employees_file, "w", encoding='utf-8') as f:
    ...
    def remove_data_async(self, sample_id, user, message_queue=None):
        threading.Thread(target=self._remove_from_databases, args=(sample_id, user, message_queue), daemon=True).start()

    def _remove_from_databases(self, sample_id, user, message_queue):
        lock = FileLock(self.lock_path, timeout=5.0)
        try:
            with lock:
                day_only = datetime.now().strftime("%d")
                ...
                final_log_row = [day_only, current_time, "ACTION: REMOVED", sample_id, removed_row[4], removed_row[5], user]
                history_exists = os.path.exists(history_file)
                with open(history_file, mode='a', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    if not history_exists:
